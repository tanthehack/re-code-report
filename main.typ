...
#show: content => bach(
  ...
  abstract: [
    // introduction, statement of problem, method, results and conclusions.

  In today's software development industry, high-quality code relies heavily on following programming conventions. When code quality suffers, it highlights the importance for developers to understand the rationale behind these conventions. This project focuses on enriching code comprehension and quality, particularly benefiting beginner to intermediate programmers. In order to achieve this, this study identified the challenges faced by novice programmers in learning programming conventions, gathered coding standards data, designed and developed an implementation of _Re-code_, an intelligent code review tool. _Re-code_ was evaluated by engaging users with questionnaires on its usability, user experience, the quality of its suggestions, and other factors. The findings highlight the system's effectiveness in providing comprehensive feedback, with over 80% of users finding it clear and helpful. This stands in contrast to traditional tools that rely on identifying predefined code issues. In using an LLM ensures scalable and contextually relevant feedback, offering substantial benefits for learning and code quality enhancement. This project lays a robust groundwork for future advancements in intelligent code review systems, contributing to improved programming practices and enriched learning environments.
  ],
  dedication: [
...
The evolution of coding standards and best practices has deep roots, dating back to the early days of programming @structured. As software development methodologies and technologies have advanced, so have coding standards. This co‐evolution is particularly evident when examining the shift from structured programming in the 1960s to object‐oriented programming in the 1980s @pratap_evolution_2023. Early coding standards or programming conventions emerged alongside structured programming to promote modular, readable code and followed a clear control flow; these characteristics became essential as software projects grew in complexity @oviedo_1984. Structured programming approaches like top-down design and control flow statements (if statements, loops) rely on well formatted code to be easily understandable and maintainable @kostyrko_learning_2023. Object‐oriented design principles like encapsulation and code reusability are best achieved through consistent naming conventions and code organization.

Among software engineering techniques, programming conventions play a critical role in promoting the readability of source code @Oliveira_2020. These conventions, which can encompass aspects like indentation, naming conventions, and code formatting, act as a shared language for developers, allowing them to quickly grasp the purpose and structure of the code, even if written by someone else @chen_automated_2018. According to #cite(<kourie_reflections_2008>, form: "prose"), there is a need to formally educate students to become professional, responsible, and reliable developers capable of producing quality code. However, #cite(<wiese_teaching_2017>, form: "prose") identifies a gap in the existing literature directed at teaching programming conventions during the initial stages of programming education. A concerning find, as poorly formatted and inconsistently named code becomes a significant hurdle as projects grow in size and complexity, hindering both understanding and maintenance. If early adoption of programming conventions is prioritized, beginner programmers can lay a solid foundation of clean, maintainable, and collaborative code throughout their software development journey @keuning_tutoring_2021.

One potential solution to bridge this gap and encourage early adoption of programming conventions is through intelligent code review tools @kim_understanding_2022. These tools are already widely used in professional software development to automate tasks like identifying syntax errors and potential bugs @sadowski_modern_2018. In the context of learning conventions, these tools can be leveraged as a valuable learning resource for beginners. According to #cite(<bancroft_program_2006>, form: "prose"), feedback or recommendations are essential in the learning process, especially when they are available on request. Valuable feedback that can improve proper understanding of programming conventions can be provided by a code reviewing tool that not only highlights potential errors but also flags sections that deviate from established coding standards. This real time feedback can raise beginners’ awareness of the importance of proper formatting, naming conventions, and code organization. By integrating educational resources and suggestions directly into the development workflow, intelligent code reviewing tools can become powerful allies for beginner programmers, helping them write clean, maintainable, and well-structured code from the very beginning.

While established code reviewing platforms like those offered by GitHub and Codacy are valuable tools in the programmer’s arsenal, they have a blind spot, programming conventions. These platforms primarily focus on identifying functional errors and bugs, ensuring the code works as intended @badampudi_modern_2023. However, they often fall short in addressing whether the code adheres to established programming conventions. Some might offer essential error messages related to “code smells” or stylistic inconsistencies but lack the depth and interactivity needed to be a true learning resource for beginners who need to understand the “why” behind coding conventions @wessel_what_2020. This gap in feedback leaves aspiring programmers vulnerable to developing bad habits and writing code that becomes a tangled mess down the line.
...
In the early days, 1940s 1960s, of languages like FORTRAN and COBOL, conventions focused primarily on basic formatting and readability due to limited processing power and the linear nature of these languages @medoff_evolution_2015. Clear documentation was crucial for maintaining code written on punch cards, so conventions often encouraged detailed comments and code structure mirroring the documentation layout.

With the rise of procedural programming languages like C in the 1960s 1980s, conventions evolved to address modularity and control flow. Indentation became a cornerstone for defining code blocks and functions, promoting better organization and readability @kernighan_c_1978. As code sharing and collaboration increased, platform specific conventions emerged to ensure portability and maintainability across different systems @pressman_software_2014.

The shift to object oriented languages like C++ and Java in the 1980s 2000s necessitated entirely new conventions for class structures, inheritance, and polymorphism @meyer_object-oriented_1997. Community driven coding standards like PEP 8 for Python, @van_rossum_pep_2001, or Google’s JavaScript Style Guide further emphasized code consistency within specific programming communities. These trends continue in the modern age, with conventions focusing on maintainability and scalability in complex, distributed software projects @beck_extreme_1999. Integration with code analysis tools further enforces adherence to conventions, ensuring clean, maintainable code remains a priority regardless of the programming language or paradigm used.
...
MCR’s process, as explained in the works by #cite(<badampudi_modern_2023>, form: "prose"), follows a series of six main steps, shown in figure 2.2, that integrate with version control systems (e.g., Gerrit, GitHub and GitLab). The steps involved in this process include:
...
The conceptual design of the system outlines the high level structure and components of the system. This section shows the Entity Relationship Diagram (ERD) for the proposed system, detailing the entities and their relationships. The conceptual diagram is shown in the figure 3.2 below.
...
These are the requirements necessary for the hardware components to run the system. The hardware requirements for the proposed system are shown in the table 4.1:
...
These are the requirements needed by the programs of the system to run successfully. They must be pre installed and set for the proposed system to run smoothly. The software requirements for the proposed system are shown in the table 4.2:
...
In addition to the questionnaires conducted, a comparison of the intelligent code review system to other reviewed systems was conducted. The comparison was based on the features, limitations, and advantages of the reviewed systems, including FrenchPress, Refactor Tutor, and Codacy and it highlighted the unique strengths of the intelligent code review system and how it addressed the limitations of the reviewed systems.
...
An intelligent code review tool is a system allowing its users to receive dynamic suggestions to code violations found within the review process. Such a tool is very helpful in improving code quality and maintaining programming conventions in the software development industry. So, the use of this system in the early stages of learning to code can not only improve developers' code quality but also improve a programmer’s code comprehension. To address this,  pretrained Large Language Model was used in this project to provide contextual explanations on code violations found during the review process. The Static Analysis techniques are used in the project to identify the code violation in the user-submitted code.

During the evaluation, it was observed that the use of LLM and static analysis techniques offered a comprehensive approach to code review. In testing the tool in a variety of scenarios, results were effective at improving the code quality and user understanding: 71.5% responded in the questionnaires that the system's quality of suggestions surpasses those of the human reviewers, and another 71.4% believed that the feedback delivered from the system was clear. In general, the dynamic feedback mechanism of the system, combined with its coverage of code violations, is making it different from those of traditional tools like the FrenchPress plugin. The power of this intelligent code review tool really lies in the adaptiveness of contextually relevant feedback it provides, which ultimately makes learning very empowering for programmers at all levels, contributing thus to better software development outcomes.
...
With a background in enhancing code quality and improving conformance to coding conventions, the development and implementation of the intelligent code review tool has marked a step-change in the way that one can approach these questions in the software development industry. This project has shown the ability to give dynamic and contextual suggestions of code violations through powerful static analysis techniques for a pre-trained Large Language Model (LLM), supporting developers at any skill level to write better quality code. The emphasis of the project was placed on carrying out deep research and understanding of LLM in order to give accurate and informative feedback on the quality of the code. By overcoming traditional approaches with its adaptive feedback mechanisms, the tool has not only identified a wider range of coding issues but also educated its user on the best way of learning programming standards.

In future, practical applications of the tool further streamlined code review processes, enabled real-time analysis, and actionable insights into codebases. This has helped heighten the satisfaction of users, make their works more effortless with developer productivity, and maintenance of codes due to the intuitiveness in the user interface and responsiveness of the system feedback. Over time, as digital settings continue to change, the intelligent code review tool is going to be the floor on which further innovations in the software development practice shall lie. Addressing the current limitations and even foreseeing future challenges, this project opens up possibilities for further advancement in automated code analysis and educates the young to be conscious about quality and efficiency in the software development ecosystem.